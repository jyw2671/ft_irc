...
void IRCCommand::m_mode_sign(const char c)
{
	_channel->reserve_sign(c);
}

void IRCCommand::m_mode_valid(const char c)
{
	_channel->reserve_flags(c);
}

void IRCCommand::m_mode_invalid(const char c)
{
	if (_ascii[(int)c])
		return;
...
e_result IRCCommand::parse_flag(const std::string& flag)
{
	std::string result;
...
e_result IRCCommand::m_mode(e_phase phase)
{
	if (phase == ONE)
	{
	...
	else if (phase == FOUR)
	{
		if (!_map.client.count(*_target))
			...
			return (m_to_client(err_users_dont_match(_request->parameter.size() == 1 ? "view" : "change")));
...
void IRCCommand::mode()
{
	...
	if (m_is_valid(CHANNEL_PREFIX))
	{
		if (m_mode(TWO) == ERROR)
			...
			m_to_client(rpl_channel_mode_is(_channel->get_name(), _channel->get_status()));
	}
	else
	{
...
void IRCCommand::m_mode_initialize()
{
	for (int i = 0; i < 127; ++i)
